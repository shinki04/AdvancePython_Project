...
class TableApp():
    ...
    def widgets_manage(self):
        ...
        # # Tạo thanh cuộn
        # scrollbar = ttk.Scrollbar(self.table_frame, orient=tk.VERTICAL, command=self.data_table.yview)
        # self.data_table.configure(yscrollcommand=scrollbar.set)  # Kết nối thanh cuộn với Treeview
        ...
        add_button = ttk.Button(form_frame, text="Thêm hàng mới", command=self.add_data_button)
        ...
        delete_input_button = ttk.Button(form_frame, text="Xóa dữ liệu trong ô mssv", command= self.delete_data_button)
        ...
        # Nút update data   
        update_button = ttk.Button(form_frame, text="Cập nhật nội dung trong ô", command=self.update_data_button)
        update_button.grid(column=5,row=2,padx=5, pady=10)
    ...
    def update_data(self):
        try:
            update_query = sql.SQL("UPDATE {} SET hoten = %s WHERE mssv = %s").format(sql.Identifier(self.table_name.get()))
            data_to_update = (self.hoten.get(),self.mssv.get())
    ...
    def check_exist(self, mssv):
        ...
        for child in children:
            # Lấy dữ liệu của từng hàng (ở đây giá trị MSSV có thể nằm trong cột đầu tiên)
